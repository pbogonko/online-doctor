     ...
     <div className="max-w-[1170px] px-5 max-auto">
        ...
        {!loading && error && <Error errMessage={error}/>}
        {
            !loading && !error &&   (
                ...
                >
                    <div>
                     <button onClick={()=>setTab('bookings')} className={`${tab==='bookings' &&'bg-primaryColor  text-white font-normal' } p-2 mr-5 rounded-md text-headingColor font-semibold text-[16px] leading-border border border-primaryColor`}>My booking</button>
                   <button onClick={()=>setTab('settings')} className={`  ${tab==='settings' && 'bg-primaryColor  text-white font-normal'} py-2 px-5 rounded-md text-headingColor font-semibold text-[16px] leading-border border border-primaryColor`}>profile settings</button>
                   {
